...
STATE_TO_ID = {"no": 0, "partial": 1, "full": 2}
ID_TO_STATE = {v: k for k, v in STATE_TO_ID.items()}


def load_index(index_csv, task="task1"):
...
def build_samples(
    ...
    for row in scenario_df.itertuples(index=False):
        ...
        frames_df["target_time"] = frames_df["frame_time"] + float(dt)

        ann_path = dataset_root / row.annotation
        ann_df = pd.read_csv(ann_path)
        ann_df = ann_df.rename(columns={"timestamp": "label_time"})
        ann_df["label"] = ann_df["state"].map(STATE_TO_ID)
        ann_df = ann_df.dropna(subset=["label"]).sort_values("label_time")

        aligned = pd.merge_asof(
            frames_df.sort_values("target_time"),
            ann_df[["label_time", "label"]].sort_values("label_time"),
            left_on="target_time",
            right_on="label_time",
            direction="nearest",
